...
const getClient = () => {
	...
	if ((import.meta.env.MODE == 'production') || (process.env.MODE == 'production')) {
		...
		const useRemote = import.meta.env.VITE_USE_REMOTE_DYNAMODB === 'true' || process.env.VITE_USE_REMOTE_DYNAMODB === 'true';

		if (useRemote) {
			console.log('Etc DynamoDBClient (Remote)');
			const region = import.meta.env.VITE_AWS_REGION || process.env.VITE_AWS_REGION || 'ap-northeast-1';
			client = new DynamoDBClient({
				region
			});
		} else {
			console.log('Etc DynamoDBClient (Local)');
			client = new DynamoDBClient({
				region: 'us-east-1',
