<script lang="ts">
	...
	const headerItems: string[] = ['選択', '名称', 'メッシュ数', '最新更新日'];
	...
	const deleteAreaPreset = async (): Promise<void> => {
		isOverlay = true;
		...
		await submitDynamicForm('/deleteAreaPreset', fields);
	};

	// 子コンポーネントからのデータを受け取る関数
	const moveAreaPreset = async (presetInfoList: PresetInfoList) => {
		console.log('moveAreaPreset', presetInfoList);
		isOverlay = true;
		await goto(`areaPresetEditInfo/${presetInfoList.id}`);
	};
...
<h1 class="p-10 text-2xl font-semibold">地域プリセット一覧</h1>
<div class="flex flex-col items-center">
	<div class="flex gap-2">
		<Button onclick={() => handlePath('areaPresetEdit')}>地域プリセットの登録</Button>
	...
	<PresetList
		...
		onMoveAreaPreset={moveAreaPreset}
	/>
