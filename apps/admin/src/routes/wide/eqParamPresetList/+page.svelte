<script lang="ts">
	...
	const headerItems: string[] = ['選択', '地震動', '最新更新日'];
	...
	const deleteEqPreset = async (): Promise<void> => {
		isOverlay = true;
		console.log('deleteEqPreset');
		const selectedItems = (presetListRef?.getSelectedItems() as PresetInfoList[]) ?? [];
		if (selectedItems.length === 0) return;

		const fields = { id: R.pluck('id', selectedItems).join(',') };
		console.log('deleteEqPreset', fields);
		await submitDynamicForm('/deleteEqPreset', fields);
	};

	// 子コンポーネントからのデータを受け取る関数
	const moveEqParamPresetEdit = async (presetInfoList: PresetInfoList) => {
		console.log('moveEqParamPresetEdit', presetInfoList);
		isOverlay = true;
		await goto(`eqParamPresetEditInfo/${presetInfoList.id}`);
	};
...
<h1 class="p-10 text-2xl font-semibold">地震動プリセット一覧</h1>
<div class="flex flex-col items-center">
	<div class="flex gap-2">
		<Button onclick={() => handlePath('eqParamPresetEdit')}>地震動の登録</Button>
	...
	<PresetList
		...
		onMoveAreaPreset={moveEqParamPresetEdit}
	/>
