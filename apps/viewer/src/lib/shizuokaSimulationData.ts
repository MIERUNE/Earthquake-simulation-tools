...
export const districts: District[] = [
  { id: 'aoi', name: '葵区', center: [138.383, 35.015] },
  { id: 'suruga', name: '駿河区', center: [138.403, 34.955] },
  { id: 'shimizu', name: '清水区', center: [138.489, 35.016] }
];
...
function generateBuilding(id: number, districtId: DistrictId): Building {
  ...
  // 被害レベルは正規分布に近い形で生成（軽微な被害が多い）
  const damageRand = Math.random();
  let damageLevel: DamageLevel;
  if (damageRand < 0.3) damageLevel = 0;
  else if (damageRand < 0.6) damageLevel = 1;
  else if (damageRand < 0.8) damageLevel = 2;
  else if (damageRand < 0.95) damageLevel = 3;
  else damageLevel = 4;
...
export const shizuokaSimulationData = {
	// 建物データ
	buildings,
